  ...
  try {
    ...
    for (const rung of rungs) {
      while (opened < rung) {
        ...
        await expect.poll(async () => (await page.evaluate(() => window.strata.getState())).tabs.some(tab => tab.path === path)).toBe(true)
      ...
      await page.getByRole('button', { name: 'Docs menu', exact: true }).click(); await page.getByRole('menuitem').filter({ hasText: basename(paths[0]!) }).click()
      ...
      await page.getByRole('button', { name: 'Docs menu', exact: true }).click(); await page.getByRole('menuitem').filter({ hasText: basename(target) }).click()
      await expect(editor).toBeVisible({ timeout: 60_000 })
