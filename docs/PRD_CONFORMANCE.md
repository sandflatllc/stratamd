...
| 6.9 | Newest-first messages regardless of sender in both placements and This passage; reopen at the newest message; incoming messages follow the top only when already there, otherwise preserve reading position; composer stays at the bottom. | `E` `test/e2e/conversation-order.spec.ts` |
