...
| 6.9 | Chronological messages in both placements and Comment discussion; reopen at the bottom; incoming messages and streaming follow the bottom only when already there, otherwise preserve reading position; Latest response aligns the last agent message at the top, including short and folded answers; Newest returns to the bottom; composer stays at the bottom. | `E` `test/e2e/conversation-order.spec.ts` |
