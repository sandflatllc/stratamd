...
| 6.1-11 | Exact `mermaid` fences render from a bundled lazy dependency under strict security with editable source access, scoped zoom/pan, token-based theming, no Markdown mutation, and a source-preserving failure state. Pointer pan suppresses selection and native drag; diagram Ctrl+wheel/pinch does not change pane zoom; ordinary wheel still scrolls the document. | `U` `test/unit/document-intelligence.test.ts` exact classification, fixed security config, and renderer-only compatibility transforms; `DI` label-origin pointer pan, focus, selection/native-drag suppression, keyboard/toolbar/scoped-wheel zoom, ordinary scrolling, source editing, failure/theme/bytes/network; `M` `test/performance/document-intelligence-proof.spec.ts` real-review proof |
...
| 6.7-02 | Send enables only for unsent activity some recipient can receive; a segment attributed to the sole attached agent alone does not enable, its verdict does. Since cockpit phase 2, a held draft also enables Send while an attachment can receive it; decided in `docs/plans/open/cockpit-v1-plan.md` §5.5, PRD amended in phase 11. | `E` enablement matrix; `U` predicate, integration flow |
| 6.7-03 | Composer accepts an optional free-text note. | `A01` |
| 6.7-04 | Composer selects all agents by default and shows a checklist for multiple agents. Since cockpit phase 2, one recipient is preselected: the Lead, else the active conversation; a second is a deliberate click. Decided in `docs/plans/open/cockpit-v1-plan.md` §5.6, PRD amended in phase 11. | `A15`; `SC` recipient deselection; `test/unit/send-composer.test.ts` "preselects only the active conversation"; `test/e2e/cockpit-drafts.spec.ts` scenario 7 |
...
| 6.9-22 | Explorer, editor, and right rail zoom text independently by hovered pane via Ctrl/Cmd+=/−/wheel within 0.5–2.0; a Mermaid viewport consumes Ctrl+wheel/pinch for diagram zoom without changing editor text; window zoom is disabled; one `Reset zoom` text button restores 1.0 and appears only while zoomed. | `U` factor clamp/normalize; `SC` hover-targeted shortcuts, reset, restart; `DI` Mermaid-scoped wheel and unchanged pane text size |
...
| 6.9-33 | Start walkthrough selects Contents; Leave exits it; H2 is the default step set, H2+H3 is optional, and visually distinct per-heading include/remove controls drive progress without parsing heading numbers. | `U` walkthrough step selection and progress; `E` `test/e2e/walkthrough.spec.ts` controls, mode, inclusion, and empty-step state |
...
| 6.9-35 | Each eligible Contents row directly toggles between Reviewed and Revisit through its review-status checkbox, while the bottom bar exposes both explicit states for the current step. Reviewed stores SHA-256 of the exact heading section; any user, agent, outside, suggestion, Keep/Revert, undo, or redo path changes it to Revisit on mismatch and restores Reviewed only on an exact hash match. Manual Revisit retains a prior reviewed hash. | `U` incremental section tracker and every mutation path; `A` application ingress/undo scenarios; `E` direct checkbox interaction and visible marker transitions |
