...
| 2 | The preview pill and the right window; Fit window edge to edge; a device size as a narrowed viewport with the page seeing its full size; state kept while a document is centered; a Strata dialog hiding the page and giving focus back; an agent opening its own dashed tab, filling a form, and snapshotting it with the badge following; a click in an agent tab stopping its wait, Resume replaying nothing, Watch not pausing. | `E` `test/e2e/preview-window.spec.ts` |
