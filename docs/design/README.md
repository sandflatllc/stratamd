...
## Layout (app shell)
Full-viewport dark canvas; three floating working regions over it in one flex row. **All regions are user-resizable**: the two gutters between columns carry vertical pill drag-handles (left navigation 160–340px, right collaboration column 240–440px; editor takes the rest); one horizontal pill handle below the upper review window sets its height (it scrolls internally once constrained); a vertical pill handle at the top-right of the document column sets the text measure (620–1600px, stays centered — widen for tables). Persist all sizes in settings. The 2026-09-02 structured-reading handoff supersedes the prototype's three independently stacked right-rail windows.
...
2. **Left navigation window** 212px default: application-owned **Files** and **Contents** tabs. Files contains the existing Scan (mint), Refresh, folder tree (md files only per PRD §6.4), active-file tint, missing-file treatment, recents, and footer. Contents shows the live document heading hierarchy, highlights the current heading, and centers a heading when activated. Both tabs use explorer zoom and the selected tab is private per-document state.
...
4. **Right collaboration column** 300px default: one upper review window with application-owned **Changes** and **Annotations** tabs, followed by a separate persistent **Agents** window. One review tab is visible at a time and carries its visible count. While Annotations is selected, Pin Changes may show a capped summary strip inside that same window; it is not independently resizable, and a summary opens Changes. The upper window is resizable; Agents fills the remaining height and never becomes a review tab. The save-state line stays below the windows. All three surfaces use right-rail zoom.

Tab hosts use the existing rounded-window geometry and the faithful-shell prototype's compact treatment. Their tablists sit at the top of the window; the active tab uses pink text with a purple-to-pink underline, while inactive tabs are quiet text buttons. Tabs expose `aria-selected` and linked tabpanels and support ArrowLeft/ArrowRight/Home/End. Stable tab and tabpanel names are "Files", "Contents", "Changes", and "Annotations"; the separate "Agents" landmark remains visible.
...
Walkthrough controls live below the Contents heading, never in a new rail or document block. The inactive state has one **Start walkthrough** action. The active state shows `Section 3 of 12`, Previous/Next, Reviewed/Revisit, **Leave walkthrough**, and a compact H2 / H2 + H3 choice. Eligible H2/H3 outline rows gain an include checkbox while walkthrough is active; excluded rows stay in the outline at quiet opacity so they can be restored. Status uses the semantic control colors—mint for Reviewed and amber for Revisit—and is repeated in text, not color alone. When every step is excluded, progress reads `No sections included` and the outline controls remain available.
...
Each GFM table sits in a rounded table block with a compact control row above it. The first controls are **Table**, **Focus row**, and **Compare**. Table is the only editable presentation. Sort, Filter, Columns, Density, and Focus stay visible when the block is at least 620px wide; below that width they move into one **Table options** disclosure. The active presentation and focused controls use the primary lavender treatment. Read-only state is stated in text beside **Edit**, not communicated by tint alone.
...
**Focus** hides the other document blocks only inside the center editor. It changes to **Exit focus** while active. Files or Contents, Changes or Annotations, and Agents remain visible. A derived table with hidden review work shows `N review items hidden`. A review jump temporarily replaces the derived view with the editable table, adds **Return to table view**, and centers the exact mark. Choosing that action or navigating to another target restores the stored derived view.

**Discuss row** and **Discuss cell** are in the table controls once a body cell is current. Either action opens the existing annotation composer with Question selected. The quoted text is the exact Markdown row. Cell discussion also shows the selected column label in the composer context. It creates the same floating thread used by every other question.
...
Registered components are full-width Strata Vivid document blocks inside the existing wide editor measure. Each has a compact, non-editable eyebrow naming its semantic type and a directly editable Markdown body. `Callout` uses the existing informational, warning, implication, or supporting control colors as a left accent and soft inset surface. `Verdict` gives the controlling judgment a stronger outcome accent without replacing its Markdown heading hierarchy. `MetricStrip` lays an ordinary short list out as responsive metric cells while retaining list semantics and returns to a single column when narrow. `PhaseBoard` gives phase headings and their tables one shared outlined surface; it does not add progress state or application controls. Invalid registered syntax is a danger-accented source-only card with “Component needs attention,” the component name, the first plain problem, and the preserved source. Unknown tags retain the existing raw-source presentation. Components never narrow prose, create shell tabs, or introduce author-selected colors or dimensions.
...
Phase 8 keeps that same block shell. `DecisionMatrix` emphasizes its Criterion rows and offers a quiet alternative-column focus without hiding the editable source table permanently. `BeforeAfter` gives its two H3-led blockquotes equal cards separated by a directional divider, stacking them in source order below 620px. `EvidenceChain` distinguishes Claim, linked Evidence, and Therefore with one connected accent; keyboard or pointer focus on a part highlights its counterpart without inventing navigation. `Timeline` is deliberately absent: Mermaid owns static timelines and an ordinary table owns filterable status.

`Chart` places a read-only canvas and compact legend above its still-editable source table. Line and bar use prepared table data, no animation, and the six ordered `visuals.category-*` swatches; axes and labels use document/table/surface theme jobs. The chart module loads only when a valid Chart is present, failure leaves the table visible, and no chart affordance changes source.

`AnnotatedScreenshot` keeps the one ordinary local image as its only decoded bitmap. Numbered pin buttons overlay that same image; the exact five-column table becomes a compact note list visually but remains editable Markdown. **Place pin** arms one image click, pin/note focus highlights both, and **Focus image** enlarges that same inline image without opening a second inspection surface. Activating a pin opens the ordinary Question composer with the full pin row and readable screenshot context. A warning above the image says `Image changed — verify pin positions` until **Positions are correct** records the current version in every row.
...
This is a persistent window below the upper review tabs, never a tab itself. Its header reads **Agents** and always shows the attached count/status; it remains visible while either review tab or Pin Changes is in use.
...
Prototype state to reproduce: active document tab, visual/source view, per-hunk status (`pending | kept | reverted | mixed`), per-doc pending counts (drive tab badges), task checks, question thread (replies, resolved), selection menu + compose popover, user annotations, composer (note, recipients, include-external, preview tab, queued notice), agent states, modal states (conflict, recovery, revert-confirm, close-tab), banner state, panel sizes (explorer/rail widths, one `upperReviewHeight`, document measure), toast, saved-ago. Files/Contents and Changes/Annotations selections are per-document state in `reading.json`; Pin Changes is session-only. In production collaboration state comes from the main process over IPC (ghost store, segments, attachments — PRD §6.11 state table); the visual states and transitions are what this design specifies.
