...
**Discuss row** and **Discuss cell** are in the table controls once a body cell is current. Either action opens the existing annotation composer with Question selected. The quoted text is the exact Markdown row. Cell discussion also shows the selected column label in the composer context. It opens the same Thread tab used by every other question.
