...
window.Proposals = (() => {
  ...
  function defaults(state){
    ...
    if(state==='computer')group.innerHTML=select('Default account',['Codex work','Claude main'])+select('Default model',['GPT-5.6','Provider default'])+select('Thinking',['High','Medium','Low'])+select('Working copy',['Current checkout','New worktree'])+note('Applies to new conversations unless a project overrides it.');
      ...
      group.innerHTML='<div class="proposal-effective"><strong>Account and model</strong><span>Codex work · GPT-5.6</span><small>'+(state==='override'?'Project override':'Inherited from computer')+'</small>'+button(state==='override'?'Reset model and thinking':'Override model and thinking',state==='override'?'project':'override')+'</div>';
      if(state==='override')group.innerHTML+=select('Thinking for Cockpit project',['Medium','High','Low'],'Saved with account and model. Computer default is High.');
      else group.innerHTML+='<div class="proposal-effective"><strong>Thinking</strong><span>High</span><small>Uses the account and model default</small></div>';
      group.innerHTML+='<div class="proposal-effective"><strong>Working copy</strong><span>Current checkout</span><small>Inherited from computer</small>'+button('Override working copy','override')+'</div>'+note('Model and thinking are saved together. These choices apply only to new conversations in Cockpit project.');
    }
  ...
  function importFlow(state){
    ...
    if(state==='sources'){body='<fieldset class="setup-fields"><h3>Find native agent history</h3>'+note('The engine searches its configured Codex and Claude sources. Import copies history without starting an agent.')+'</fieldset>';footer+=button('Find projects','select',true);}
    if(state==='select'){body='<fieldset class="setup-fields"><h3>Choose projects</h3>'+select('Show recent projects',['20','50','100'])+'<h3>Cockpit repository</h3>'+row('Cockpit project','/tmp/cockpit · Codex, Claude · 2 conversations')+note('Destination: Existing project at this folder')+'</fieldset>';footer=button('Back','sources')+button('Review selected projects','confirm',true);}
    if(state==='confirm'){body='<div class="proposal-stack"><h3>Up to 2 conversations across 1 project</h3><p>The engine imports recent history for each selected folder, up to 100 conversations and 200 messages per conversation. Its size limits may reduce this further. Source history stays where it is.</p>'+status('Cockpit project · /tmp/cockpit<br>Destination: Cockpit project')+'</div>';footer=button('Back','select')+button('Import conversations','progress',true);}
    if(state==='progress'){body='<div class="proposal-stack"><h3>Importing projects</h3><p>0 of 1 projects complete</p><progress aria-label="Importing Cockpit project"></progress>'+note('Importing /tmp/cockpit. Counts update when the project finishes.')+'</div>';footer='';}
    if(state==='partial'){body='<div class="proposal-stack"><h3>1 imported or already present · 1 project needs attention</h3>'+status('/tmp/inspection-copy could not be read. Permission denied. The imported conversation is ready to open.','error')+note('Retry revisits failed projects without creating duplicate conversations.')+'</div>';footer=button('Open imported work','done')+button('Retry failed import','progress',true);}
    if(state==='done'){body='<div class="proposal-stack"><h3>2 conversations imported or already present</h3><p>0 conversations skipped</p>'+status('Imported work is available in Projects. No agent was started.','good')+'</div>';footer=button('Import more','sources')+button('Open imported work','drafts/markers',true);}
    dialog(title,'Copy history from an existing agent home into Strata.',body,footer,'Project import inside Strata’s existing setup dialog');
  }
  function evidence(state){
    ...
    const text=state==='saved'?'Saved locally · PNG · 838 × 815':state==='transfer'?'Copying to the agent environment':'Could not copy to the agent environment';
    add(q('.conversation-message.assistant'),`<div class="proposal-evidence">${state==='saved'?'<img src="reference/inspection-capture.png" alt="Inspection page screenshot">':''}<div><strong>${state==='saved'?'inspection-page.png':'inspection-review.webm'}</strong><p class="proposal-note">${text}</p>${state==='transfer'?'<progress aria-label="Copying to the agent environment"></progress>':''}${state==='failed'?note('Destination: Cockpit engine. The completed recording is still saved on this computer.'):''}<div>${button(state==='saved'?'Open screenshot':state==='transfer'?'View completed copy':'Retry copy',state==='saved'?'preview':state==='transfer'?'saved':'transfer')}${state==='failed'?button('View completed copy','preview'):''}</div></div></div>`,'Evidence card with saved, transfer, or retry state');
  }
  ...
  function models(state){
    ...
    const panel=add(fieldset,`<section class="proposal-stack proposal-model-editor"><h3>Inspection reviewer</h3>${tabs('Model editor',[['edit','Details'],['options','Options']],state==='edit'?'edit':'options')}${state==='edit'?field('Display name','Inspection reviewer','Shown in model lists and the composer.'):`<label class="setup-field">Reasoning effort<select aria-label="Reasoning effort" ${state==='invalid'?'aria-invalid="true"':''}>${state==='invalid'?'<option selected disabled>lots (unsupported)</option>':''}<option ${state==='options'?'selected':''}>High</option><option>Low</option></select><small>Reported as supported by this provider.</small></label><button type="button" role="switch" class="setup-switch" aria-label="Fast mode" aria-checked="false"><span aria-hidden="true"><i></i></span>Fast mode</button>`}${state==='invalid'?'<p class="proposal-invalid" role="alert">Choose a supported value for Reasoning effort in Options.</p>':''}${note(state==='edit'?'Provider ID: gpt-5.6. Renaming does not change the model sent to the engine.':'One option from another client is preserved. This provider has not reported an editor for it.')}${button('Save model',state==='edit'?'options':state==='invalid'?'options':'list',true,state==='invalid')}${button('Back to models','list')}</section>`,'Custom model name, supported options, and validation');
    panel.querySelector('[role="switch"]')?.addEventListener('click',e=>{const control=e.currentTarget;control.setAttribute('aria-checked',String(control.getAttribute('aria-checked')!=='true'));});
  }
