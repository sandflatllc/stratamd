...
window.Proposals = (() => {
  ...
  function importFlow(state){
    ...
    if(state==='partial'){body='<div class="proposal-stack"><h3>1 imported · 1 project needs attention</h3>'+status('/tmp/inspection-copy could not be read. Permission denied. The imported conversation is ready to open.','error')+note('Retry skips successful projects and conversations already imported.')+'</div>';footer=button('Open imported work','done')+button('Retry failed import','progress',true);}
    if(state==='done'){body='<div class="proposal-stack"><h3>2 conversations imported</h3><p>0 unchanged conversations skipped</p>'+status('Imported work is available in Projects. No agent was started.','good')+'</div>';footer=button('Import more','sources')+button('Open imported work','drafts/markers',true);}
