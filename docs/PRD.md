...
Status: draft v27 · 2026-09-04 · personal Linux and macOS tool
...
- The left window has Projects and Contents while a conversation is in the center, and Projects, Conversation, and Contents while a document is in the center. The top bar shows pinned documents and conversations, and the active one of each, as pills; every other open item waits in the Docs or Conversations dropdown pill. The right window has Changes, Items, and Attached. Conversation is one component in either placement. Completed agent prose is visible by default, with explicit heading folds. Each stretch of work collapses independently into a Worked for disclosure.
- Projects renders active threads in collapsible T3 project folders, and future-snoozed and explicitly settled threads in separate collapsed shelves. Pinned threads sort first and do not count toward the folder preview cap. Rows show only the star, one state indicator, title, relative time, pending-work and attachment marks, and hover Settle. The indicator resolves one state per row in T3's order (decided 2026-09-04): an open approval or question wraps its label in a warning pill; a failed session wraps Failed in a danger pill; a running session, or T3 background liveness `working`, is a pulsing dot; a completion the owner has not opened is a solid dot with a bold title; background liveness `monitoring`, watch loops with no agent work, is a robot glyph; otherwise nothing. Never-visited threads read as opened, as in T3. Rows under a folder indent beneath the folder icon; rows that are neither selected, favorited, nor carrying a pill or completion use a quieter text color; the selected row is a full-width pill at full strength. The folder header carries a dot for its loudest active state (waiting, working, finished) so a collapsed folder still reads. Every other action lives in the right-click menu. Engine status opens the Accounts modal and Settings reaches the same modal. Empty engine surfaces share the same server-naming disconnected line and Reconnect action.
...
- Deltas from the handoff, decided here:
  ...
  - **Conversation lives in the left window or a center tab.** The same component follows the active T3 thread. Completed agent prose stays visible by default; the owner can fold headings in either placement. All messages appear in chronological order, oldest at the top and newest at the bottom, regardless of sender. This also applies to Comment discussion, including replies and decision answers. Document review discussions remain beside the transcript; message discussions open beneath their anchored answer. There is no whole-thread versus discussion scope switch. Opening or returning to a conversation starts at the bottom; incoming messages and streaming text follow the bottom when the reader is already there, and otherwise preserve the current reading position. Latest response puts the beginning of the last agent message at the top of the viewport, revealing folded headings when necessary and allowing room below short answers. Newest returns to the bottom. Both buttons are available in either conversation placement. The composer stays at the bottom; in the center placement it shares the transcript's measure, so the one width handle sizes both. Finished work collapses into a **Worked for** disclosure that names how long the stretch ran, measured from its first call to the answer that followed it, and a line beneath it separates the work from that answer; opening it lists the calls and traces. Running work stays live with Working, elapsed time, and Thinking states. Document-anchored item activation still centers and highlights its passage. Message items open the immutable agent passage. The left window has one width for every tab, `explorerWidth` in `settings.json` (decided 2026-09-04; the older `threadPanel` width is ignored when normalizing settings). Conversation text scales with the left window's zoom factor in the side placement and with the editor's factor in the center, where the conversation is the editor pane. Messages render block-level Markdown (headings, lists, code blocks, quotes, tables) with the document's typography tokens at conversation scale; completed messages use the read-only Strata editor; streaming prose and rail snippets retain lightweight rendering. A turn's changed files fold into one card after its last agent message (decided 2026-09-04, matching T3): a count and a net delta in thousands form, the top-level folders they fall under with counts, up to three file chips named by file with an extension mark, and **Show all** or **Show files** to list every file by folder with its relative path and counts. Paths read relative to the project's workspace root and never print in full unless they lie outside it; the full path is the row's tooltip. A changed Markdown file opens as a document from its chip or row; other files list without an action. Enter sends from its compact composer and Shift+Enter inserts a newline.
  ...
  - **Contents, decided 2026-09-02.** Document Contents is derived from the live ProseMirror document. Conversation Contents uses cached canonical message parses before editor mounting. H1 is presented as the document title; H2 headings are primary sections; H3–H6 nest beneath their nearest shallower heading. Editing a heading updates the index immediately. The active row follows the editor's scroll position, and activating a row centers its heading without changing the document. A heading-free document shows a plain empty state. The selected left and review tabs are remembered per document in its private `reading.json`; switching document tabs restores them along with the editor's existing per-document scroll position.
