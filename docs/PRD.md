...
- The left window has Files, Contents, Projects, and Conversation. Documents and conversations share center tabs. The right window has Changes, Items, a conversation-scoped Documents panel, and Attached. Conversation is one component in either placement. Agent prose is always shown in full; earlier turns fold only their work log.
...
- Deltas from the handoff, decided here:
  ...
  - **Conversation lives in the left window or a center tab.** The same component follows the active T3 thread. Agent prose stays visible in both placements. Finished work collapses by turn into step toggles, while running work stays live with Working, elapsed time, and Thinking states. Document-anchored item activation still centers and highlights its passage. Message items open the immutable agent passage. The left window keeps independent navigation and Conversation widths in `settings.json`; Conversation has no fixed maximum and a 330px minimum. Enter sends from its compact composer and Shift+Enter inserts a newline.
