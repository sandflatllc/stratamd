...
Status: draft v25 · 2026-09-03 · personal Linux and macOS tool
...
- The left window has Files, Contents, Projects, and Conversation. Documents and conversations share center tabs. The right window has Changes, Items, a conversation-scoped Documents panel, and Attached. Conversation is one component in either placement; prose folds by default only in the left placement.
- Projects groups threads under their T3 projects with lifecycle state, unread status, pending item/change count, row actions, and New thread. Engine status opens the Accounts modal and Settings reaches the same modal. Empty engine surfaces share the same server-naming disconnected line and Reconnect action.
...
- Deltas from the handoff, decided here:
  ...
  - **The right rail is a map, decided 2026-08-30 and normalized for the cockpit on 2026-09-03.** Changes and Items use compact rows that center their marked document or message passage. Attached lists document-linked T3 threads with live engine state, Lead, Stop, open-as-tab, and Detach. Documents lists the active conversation's files and counts without opening them. A row names its action, author, and a short excerpt rather than an internal id or path.
  - **Conversation lives in the left window or a center tab.** The same component follows the active T3 thread. Left-window prose folds by default; center prose expands. Document-anchored item activation still centers and highlights its passage. Message items open the immutable agent passage. The left window keeps independent navigation and Conversation widths in `settings.json`; Conversation has no fixed maximum and a 330px minimum. Enter sends from its composer and Shift+Enter inserts a newline.
  ...
  - **Tabbed side hosts, normalized 2026-09-03.** The left window owns **Files**, **Contents**, **Projects**, and **Conversation**. The upper-right review host owns **Changes** and **Items**; **Pin Changes** adds a capped session-only strip while Items is selected. **Attached** remains visible below it, and the active conversation adds **Documents**. Documents and threads cannot alter application tabs. The center tab strip holds both documents and conversations. Files, Contents, and Projects keep the `explorer` pane identity; Changes, Items, Documents, and Attached keep `rightRail`; the shared Conversation component keeps its own size in either placement.
...
- Open from the explorer, CLI, file manager, or drag-drop. The file-drop overlay and messages respond only to transfers that contain files; text, HTML, images, and internal editor drags do not enter the open-file workflow.
...
- **Forget document** in the explorer deletes the entry and its unreferenced blobs.
