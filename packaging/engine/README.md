...
The Windows build stages the pinned `win32-x64` Node zip and uses its bundled npm. node-gyp builds node-pty with the target Node headers and Windows import library; ConPTY support files remain in the runtime. NSIS packages a per-user installer and `stratamd.cmd`. See [Windows setup and verification](../../docs/platforms/windows.md). Native Windows CI is required before release signoff.
