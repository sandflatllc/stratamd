...
it.skipIf(!process.env.STRATAMD_ENGINE_BUNDLE)('folder replacement upgrades and preserves identity and settings', async () => {
  const fixture = await upgradeFixture('folder replacement')
  try {
...
it.skipIf(!process.env.STRATAMD_ENGINE_BUNDLE)('rollback archives newer work and persists the restored runtime selection', async () => {
  const fixture = await upgradeFixture('rollback')
  try {
...
it.skipIf(!process.env.STRATAMD_ENGINE_BUNDLE)('a persisted runtime selection remains pinned across restart', async () => {
  const fixture = await upgradeFixture('pinned restart')
  try {
    await fixture.release(1)
...
it.skipIf(!process.env.STRATAMD_ENGINE_BUNDLE)('a failed update recovers the previous engine and settings and archives the failed work', async () => {
  const fixture = await upgradeFixture('failed update')
  try {
    await fixture.release(1)
    await fixture.start()
    await fixture.settings(15)
