...
for (const enabled of [false, true]) it.skipIf(!process.env.STRATAMD_ENGINE_BUNDLE)(`managed shutdown continuation marker and startup honor preference ${enabled}`, async () => {
  ...
  try {
    ...
    await writeFile(join(root, 'engine/t3/userdata/settings.json'), JSON.stringify({ providerInstances: { codex: { driver: 'codex', config: { binaryPath: resolve('test/fixtures/restart-codex.cjs'), homePath: join(root, 'provider-home') }, environment: [{ name: 'STRATA_RESTART_RECORD', value: record, sensitive: false }] } } }))
    ...
    await client.editProvider({ identity: client.view().identity!, instanceId: 'codex', base: settings.providerInstances.codex!, patch: { config: { binaryPath: resolve('test/fixtures/restart-codex.cjs'), homePath: join(root, 'provider-home') }, environment: [{ name: 'STRATA_RESTART_RECORD', value: record, sensitive: false }] } })
