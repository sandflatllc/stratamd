...
it('exposes only configuration supported by the driver', () => {
  ...
  expect(providerConfigFields('cursor').map(field => field.key)).toEqual(['binaryPath'])
})
it('reads legacy stores and filters damaged model preferences', () => {
  ...
  expect(normalizeAccountsStore({ formatVersion: 1, modelPreferences: { codex: { favorites: ['model', 2], hidden: ['other'] }, broken: null } }).modelPreferences).toEqual({ codex: { favorites: ['model'], hidden: ['other'] } })
})
