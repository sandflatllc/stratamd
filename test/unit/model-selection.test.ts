...
it('the designation drops the vendor word the provider glyph carries', () => {
  ...
  expect(modelDesignation({ name: 'GPT-6-Astra', slug: 'gpt-6-astra', driver: 'codex' })).toBe('Astra')
  expect(modelDesignation({ name: 'GPT-5.6-Sol', slug: 'gpt-5.6-sol', driver: 'codex' })).toBe('Sol')
  expect(modelDesignation({ name: 'GPT-5.6-Terra', slug: 'gpt-5.6-terra', driver: 'codex' })).toBe('Terra')
  expect(modelDesignation({ name: 'GPT-5.6', slug: 'gpt-5.6', driver: 'codex' })).toBe('5.6')
  expect(modelDesignation({ name: 'GPT-5.4-Mini', slug: 'gpt-5.4-mini', driver: 'codex' })).toBe('5.4 Mini')
  expect(modelDesignation({ name: 'GPT-5.4-Codex', slug: 'gpt-5.4-codex', driver: 'codex' })).toBe('5.4 Codex')
  expect(modelDesignation({ name: 'Claude Opus 5', slug: 'claude-opus-5', driver: 'claudeAgent' })).toBe('Opus 5')
