...
test('dropdowns remain clickable above the workspace, scroll, and restore keyboard focus', async ({}, testInfo) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Conversations menu', exact: true }).click()
    await expect(menu).toBeHidden()
    await expect(page.getByRole('menu', { name: 'Open conversations' })).toContainText('No conversations open')
    await page.keyboard.press('Escape')
    ...
    await page.getByRole('button', { name: 'Conversations menu', exact: true }).click()
    const conversations = page.getByRole('menu', { name: 'Open conversations' })
    await page.screenshot({ path: testInfo.outputPath('conversations-open.png') })
    await conversations.getByRole('button', { name: 'Pin Live engine thread', exact: true }).click()
    await conversations.getByRole('menuitem', { name: 'Live engine thread', exact: true }).click()
