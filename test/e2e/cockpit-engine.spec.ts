...
test('cockpit parity: Conversation keeps prose visible and groups finished and live work like T3', async ({}, testInfo) => {
  ...
  try {
    ...
    await expect(conversation.locator('.conversation-work-toggle').first()).toContainText('Ran 1 command')
    await expect(conversation.locator('.conversation-work-toggle').first()).toHaveAttribute('aria-expanded', 'false')
    await conversation.locator('.conversation-work-toggle').first().click()
