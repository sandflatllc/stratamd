...
test('3: Send now, a ready reply by revision, Looks right without a turn, Still wrong as a new revision, and a late reply that changes nothing', async ({}, testInfo) => {
  ...
  try {
    ...
    await again.getByRole('button', { name: 'Hold' }).click()
    await page.getByRole('textbox', { name: 'Message conversation' }).filter({ visible: true }).press('Enter')
...
test('Enter holds an image, the next Enter sends its sheet, and transcript images can be annotated again', async ({}, testInfo) => {
  const engine = await startEngine({ pendingRequests: false })
  const scenario = await seededScenario(testInfo, engine.origin)
  try {
    const page = await scenario.launchEmpty()
    await openLiveThread(page)
    await pasteScreenshot(page)
    const dialog = await markUp(page, 'Keep these marks and my note visible.')
    await dialog.getByRole('textbox', { name: 'Visual comment' }).press('Enter')
    await expect(dialog).toBeHidden()
    const composer = page.getByRole('textbox', { name: 'Message conversation' })
    await expect(composer).toBeFocused()
    expect(engine.commands.filter(command => command.type === 'thread.turn.start')).toHaveLength(0)
    await composer.press('Enter')
    const sheet = page.locator('.conversation-sent-images img')
    await expect(sheet).toBeVisible()
    await expect.poll(() => sheet.evaluate(image => (image as HTMLImageElement).naturalWidth)).toBeGreaterThan(0)
    const saved = await page.evaluate(async () => (await window.strata.getState()).engine.projects.flatMap(project => project.visualComments ?? [])[0]!)
    await expect(sheet).toHaveAttribute('src', saved.revisions[0]!.images![0]!)
    await sheet.click()
    const inspector = page.getByRole('dialog', { name: 'Inspect image', exact: true })
    await expect(inspector).toBeVisible()
    await page.keyboard.press('Control+=')
    await expect(inspector.getByLabel('Image zoom')).toHaveText('120%')
    await inspector.getByRole('button', { name: 'Fit', exact: true }).click()
    await expect(inspector.getByLabel('Image zoom')).toHaveText('100%')
    await inspector.getByRole('button', { name: 'Annotate', exact: true }).click()
    await expect(dialog).toBeVisible()
    await dialog.getByRole('textbox', { name: 'Visual comment' }).fill('A second note on the sent image.')
    await dialog.getByRole('textbox', { name: 'Visual comment' }).press('Enter')
    await expect(composer).toBeFocused()
    await expect(page.getByRole('button', { name: 'Remove held visual comment: A second note on the sent image.' })).toBeVisible()
  } finally { await scenario.dispose(); await engine.close() }
})
