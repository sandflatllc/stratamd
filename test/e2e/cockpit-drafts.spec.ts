...
test('8: Start thread from a document preselects its project and sends the pending comment, held draft, and document as the first turn', async ({}, testInfo: TestInfo) => {
  ...
  try {
    ...
    expect(create).toMatchObject({ projectId: 'p1', title: 'New thread' })
    expect(turn.modelSelection).toEqual(create.modelSelection)
    expect(turn.modelSelection).toMatchObject({ instanceId: 'claude-main', options: expect.arrayContaining([{ id: 'contextWindow', value: '1m' }]) })
    await expect.poll(() => engine.uploads.length).toBe(1)
    ...
    await expect(page.locator('.agent-row')).toContainText('New thread')
