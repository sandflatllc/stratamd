...
test('transcript theme controls preview independently and persist layout, shadow, and colors', async ({}, testInfo) => {
  ...
  try {
    ...
    await expect.poll(async () => JSON.parse(await readFile(themePath, 'utf8')).surfaces).toMatchObject({ 'transcript-style': 'panel', transcript: '#203040', 'transcript-border': '#607080', 'transcript-shadow-style': 'drop-shadow', 'transcript-shadow': '#ff0000', panel: '#15141a' })
    ...
    await expect(history).toHaveCSS('box-shadow', redShadow)
    await expect(page.locator('.chat-composer-box')).not.toHaveCSS('box-shadow', redShadow)
    ...
    await expect(restoredHistory).toHaveCSS('box-shadow', redShadow)
