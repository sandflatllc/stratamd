...
test('adding a display name writes a structured model while its provider ID remains explicit', async ({}, testInfo) => {
  ...
  try {
    ...
    expect((await page.evaluate(() => window.strata.readEngineSettings())).providerInstances.codex?.config).toEqual({ futureConfig: 'keep', customModels: [{ slug: 'gpt-5.6', name: 'Inspection reviewer' }] })
  } finally { await scenario.dispose(); await engine.close() }
})
