...
test('usage changes windows and metrics, groups by hour, refreshes and opens Accounts', async ({}, testInfo) => {
  ...
  try {
    ...
    await page.getByRole('menuitem', { name: 'Token Use', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: 'Token Use' })
    const initialWidth = await dialog.evaluate(element => (element as HTMLElement).offsetWidth)
    await dialog.getByRole('button', { name: 'Resize Token Use' }).press('ArrowLeft')
    await expect.poll(() => dialog.evaluate(element => (element as HTMLElement).offsetWidth)).toBe(initialWidth - 10)
    ...
    await dialog.getByRole('button', { name: 'Usage Limits', exact: true }).click()
    await expect(dialog).toHaveCount(0)
    const accounts = page.getByRole('dialog', { name: 'Usage Limits', exact: true })
    await expect(accounts).toBeVisible()
    await parityCapture(page, 'provider-overview')
    await accounts.getByRole('button', { name: 'Token Use', exact: true }).click()
