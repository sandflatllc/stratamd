...
test('connection details keep pairing collapsed and support keyboard dismissal', async ({}, testInfo) => {
  ...
  try {
    ...
    await expect(dialog.getByRole('button', { name: 'Open T3 in a browser' })).toBeVisible()
    ...
    // The named menu entry stays discoverable when the status label compacts to a dot.
    await scenario.app!.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows()[0]!.setSize(1024, 700))
    await expect(page.locator('.engine-status-label')).toBeHidden()
    await page.getByRole('button', { name: 'StrataMD menu' }).click()
    await page.getByRole('menuitem', { name: 'Engine', exact: true }).click()
