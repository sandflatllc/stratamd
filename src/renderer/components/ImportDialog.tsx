...
export function ImportDialog({ engine, onClose }: { engine: EngineView; onClose(): void }) {
  ...
  return <SetupDialog className="history-import-dialog" title="Import existing work" subtitle="Copy history from an existing agent home into Strata." onClose={close} footer={<>
    ...
    {step === 'done' && <><h3>{imported} {imported === 1 ? 'conversation' : 'conversations'} imported or already present</h3><p className="history-import-notice">Imported work is available in Projects.</p><p>{skipped} conversations skipped</p>{skipped > 0 && <p className="history-import-note">Some history is unreadable, unsupported, or beyond the engine’s import limits. Imported work is ready to open.</p>}<p className="history-import-note">No agent was started. Open a conversation to review its history before continuing it.</p></>}
  </SetupDialog>
}
