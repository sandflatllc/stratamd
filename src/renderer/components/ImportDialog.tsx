...
export function ImportDialog({ engine, onClose }: { engine: EngineView; onClose(): void }) {
  ...
  const runImport = async (retry = false) => {
    ...
    const completed = retry ? outcomes.filter(outcome => outcome.result && !outcome.error) : []
    ...
    for (const item of pending) {
      ...
      try {
        ...
        completed.push({ candidate: item.candidate, projectId, result, ...(result.skippedCount > 0 ? { error: `The engine skipped ${result.skippedCount} ${result.skippedCount === 1 ? 'conversation' : 'conversations'}. History may be unreadable or unsupported. Check the source history and retry.` } : {}) })
  ...
  return <SetupDialog className="history-import-dialog" title="Import existing work" subtitle="Copy history from an existing agent home into Strata." onClose={close} footer={<>
    ...
    {step === 'partial' && <><h3>{imported} imported or already present · {failed.length} {failed.length === 1 ? 'project needs' : 'projects need'} attention</h3>{failed.map(outcome => <p role="alert" className="history-import-notice" key={outcome.candidate.path}>{outcome.candidate.path}: {outcome.error}</p>)}<p className="history-import-note">Retry revisits projects needing attention without duplicating conversations.</p></>}
    {step === 'done' && <><h3>{imported} conversations imported or already present</h3><p className="history-import-notice">Imported work is available in Projects.</p><p>{skipped} conversations skipped</p><p className="history-import-note">No agent was started. Open a conversation to review its history before continuing it.</p></>}
  </SetupDialog>
}
