...
export function ThreadRecoveryNotice({ engine, thread, onReconnect }: { engine: EngineView; thread: EngineThreadView; onReconnect(): void }) {
  ...
  const text = state === 'reconnecting' ? 'Reconnecting to the engine… Conversation history and your draft are available.'
    ...
    : state === 'continued' ? recovery?.source === 'message' ? 'Strata sent a new continuation message. Your held answers and draft were kept.' : 'Reconnected. Work is running in a new turn.'
  ...
  return <section className="conversation-request thread-recovery-notice" data-state={state} data-testid={disconnected ? 'conversation-disconnected' : undefined} data-history-row role={state === 'failed' ? 'alert' : 'status'}>
    <span>{text}</span>
    {(state === 'failed' || state === 'waiting') && <><small>{error || recovery?.failure}</small><div className="conversation-actions"><button type="button" disabled={busy} onClick={onReconnect}>Check recovery</button><button type="button" disabled={busy} onClick={() => void send()}>{busy ? 'Sending continuation…' : 'Continue with a message'}</button></div></>}
