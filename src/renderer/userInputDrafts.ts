...
export function readUserInputDrafts(key: string): Record<string, UserInputDraft> {
  try {
    ...
    return Object.fromEntries(Object.entries(value).flatMap(([id, raw]) => {
      ...
      if (Object.values(raw).every(answer => typeof answer === 'string')) return [[id, { answers: raw, attachmentsByQuestionId: {} }]]
      const draft = raw as UserInputDraft
      if (!draft.answers || !Object.values(draft.answers).every(answer => typeof answer === 'string')) return []
