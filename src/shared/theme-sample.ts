...
What you cannot see here: the panels around this document use the **Panel background** on the **Window background**; hovered rows and chips use the **Inset and hover background**; what you type into goes on the **Text field background**; the menu that appears when you select text uses the **Popover and toast background**. **Reading panel layout** selects opaque panels or an open layout for both documents and conversation transcripts. The panel uses **Reading panel background** and **Reading panel border**. **Reading panel shadow** adds an optional drop shadow in **Reading panel shadow color**. **Reading panel shadow strength** makes it wider and darker, from 0% to 300%. Conversation messages use **User message background** and tool calls use **Tool call background**. Your annotations are **Your changes** and each attached agent has its own color. The glows and motes behind everything come from the five effect colors in Decoration and motion. **Side windows** gives all side windows animation, animation behind glass, or a solid background. **Side-window glass opacity** adjusts the glass from clear to solid without changing the center window.
`
