...
const argumentSchemas: Record<InvokeChannel, z.ZodType> = {
  ...
  [IPC.scanEngineHistory]: z.tuple([]),
  [IPC.importEngineHistory]: z.tuple([z.object({ projectId: z.string().trim().min(1), expectedWorkspaceRoot: pathSchema }).strict()]),
  ...
  [IPC.continueInterruptedThread]: z.tuple([idSchema]),
...
export function registerStrataIpc(options: RegisterIpcOptions): RegisteredIpc {
  ...
  const handlers: Record<InvokeChannel, (...args: never[]) => unknown> = {
    ...
    [IPC.scanEngineHistory]: () => options.api.scanEngineHistory(),
    [IPC.importEngineHistory]: (input: Parameters<StrataApi['importEngineHistory']>[0]) => options.api.importEngineHistory(input),
    ...
    [IPC.continueInterruptedThread]: (threadId: string) => options.api.continueInterruptedThread(threadId),
